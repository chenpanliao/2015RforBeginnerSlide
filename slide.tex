...
\renewcommand\footnote[1]{\hspace{-0.6em}\oldfootnote{\tiny\ignorespaces#1}\hspace{0.6em}}
...
\begin{enumerate}
\item 前往 \url{http://goo.gl/YRCOaN}，下載 \verb+exam.xlsx+。
...
\begin{block}{目標}
建立Science對應Literatur的簡單線性迴歸模型，並檢驗斜率是否為零。
\end{block}
\begin{block}{\texttt{lm()}基本語法}
...
\begin{block}{目標}
計算Science與Literatur的簡單線性相關係數是否為零。
\end{block}
\begin{block}{\texttt{cor.test()}基本語法}
\verb+lm(第一群資料, 第二群資料, ...)+
\end{block}
\begin{verbatim}
...
\begin{itemize}
\item 去修使用R的課會是最快的方法。
\item 多「玩」。把新學到的函數裡的 Example 玩一玩、改一改。
\item 肯問人。逛逛網路教學和論壇。只要誠心有禮都有好結果。
\item 買（可能不只一本）書。
\item 拿出統計學課本的例題，用R做做看。
\item 做過的程式碼要建檔，以後就只要小修改即可再使用。
\item 卡關時，先用英文問Google大神。很容易就找到解法。
\pause
\item 做出答案時，不要直接相信這是正解。\\
 做出答案時，不要直接相信這是正解。\\
 做出答案時，不要直接相信這是正解。\\
（因為很重要，所以要說三次。）\\
 應該以專業人士、書籍、網頁資料驗證。
...
\begin{itemize}
\item 《R軟體：應用統計方法》陳景祥著，東華出版社。\\ 對初學者很有幫助的一本。
\item 《R 錦囊妙計》 Paul Teetor著，張夏菁譯，歐萊禮出版社。\\ 內容適合需要以R進行統計工作的研究人員。
\item {\CJKfontspec{Hei}《R语言实用教程》薛毅、陈立萍著，清华大学出版社。}
\item {\CJKfontspec{Hei}《统计建模与R软件》薛毅、陈立萍著，清华大学出版社。} \\ 以數理統計為主，R語言實作為輔。
\end{itemize}
\end{frame}
...
\item ``Quick-R''\oldfootnote{\url{http://www.statmethods.net/}} by Robert I.~Kabacoff
...
\item[Q] 聽到這裡，我想認輸了$\cdots\cdots$我想重回SPSS用滑鼠搞定的世界。
\item[A] 只要是適合自己的工具，就是好工具。（當然，也要買得起才行。我就是買不起才$\cdots\cdots$）
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{請按今天的課程內容完成以下練習}
\begin{enumerate}
\item 前往 \url{http://goo.gl/YRCOaN}，下載 \verb+test.xlsx+。想辦法將它讓R可以讀取。
\item 
\end{enumerate}
\end{frame}
