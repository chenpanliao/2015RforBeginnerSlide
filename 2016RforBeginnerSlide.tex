...
\usepackage{xcolor}
\definecolor{codecolor}{rgb}{0.2, 0.5, 0}
...
\renewcommand\footnote[1]{\hspace{-0.6em}\oldfootnote{\ignorespaces#1}\hspace{0.5em}}
\setbeamerfont{footnote mark}{size=\tiny}
\setbeamerfont{footnote}{size=\scriptsize}
\setbeamertemplate{footnote}{\textsuperscript{\insertfootnotemark}\insertfootnotetext}

\setbeamertemplate{itemize item}{\large\raise-0.2pt\hbox{\textbullet}}
...
\DefineVerbatimEnvironment{RC}{Verbatim}{fontsize=\small, baselinestretch=1, samepage=true}
\DefineVerbatimEnvironment{R}{Verbatim}{fontsize=\footnotesize, frame=single, baselinestretch=1, samepage=true}

%% 西文字配置
\linespread{1.15}
\usepackage{newtxsf}
\usepackage[no-math]{fontspec}
\setmainfont[Mapping=tex-text, BoldFont={Fira Sans SemiBold}]{Fira Sans}
\setsansfont[Mapping=tex-text, BoldFont={Fira Sans SemiBold}]{Fira Sans}
\setmonofont[Color=codecolor, Scale=1, LetterSpace=0, BoldFeatures={Color=codecolor}]{Noto Sans Mono CJK JP}
\usefonttheme{professionalfonts}

%% 中文字配置
\newcommand{\nbs}{\hskip2pt plus 1pt minus 1pt}
\usepackage[
  ...
  CJKglue = {\hskip0pt}, CJKecglue = {\nbs}
]{xeCJK}
\setCJKmainfont[Scale=1, LetterSpace=0]{Noto Sans CJK JP}
\setCJKmonofont[Color=codecolor, Scale=1, LetterSpace=-0]{Noto Sans Mono CJK JP}
...
\setbeamertemplate{title page}{
  \vskip0pt plus 0.5filll
    \huge{\bfseries\usebeamercolor[fg]{title}初學\hspace{0.2em}\includegraphics[width=3cm]{Rlogo-novo.png}\hspace{0.2em}語言的120分鐘}\\[0pt]
...
\textattachfile[appearance=false, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]{nation-data.xlsx}{}
    ...
    \normalsize{廖鎮磐 \texttt{<andrew.43@gmail.com>} \\ 東海大學生命科學系}
    \vskip0pt plus 0.2filll
    {\scriptsize
        \parbox[c]{3cm}{% 
          \includegraphics[width=3cm]{cc.pdf}%
        }\hspace{6pt}%
        \parbox[c]{8cm}{% 
           © 2016 廖鎮磐（Chen-Pan Liao）。本文件採用\href{http://creativecommons.org/licenses/by-sa/4.0/deed.zh_TW}{姓名標示-相同方式分享 4.0 國際授權}（CC BY-SA 4.0），\footnote{\url{http://creativecommons.org/licenses/by-sa/4.0/deed.zh_TW}。}以Adobe Reader開啟本PDF可取得練習資料檔案附件。}% 
    \vspace{10pt}
    }
}
...
\begin{itemize}
\item 不要只是在R的命令列輸入R指令。建議以純文字編輯器撰寫R程式碼，並儲存成「\verb+.R+」檔，供未來始用時參考。
\item 「語法多色支援」、「語法提示」、「即時執行」等功能的編輯器，增加撰寫效率。\\ \includegraphics[width=0.7\textwidth]{editor.png}
\end{itemize}
\begin{description}[longest item]
	\item [\href{http://www.rstudio.com/}{RStudio}] 目前最流行的 IDE，跨平台，支援功能多。\footnote{\url{http://www.rstudio.com/}}
	...
	\item [Atom + language-r + r-exec] MAC平台上效果好。\footnote{\url{https://atom.io/}}
\end{description}
\end{frame}
...
\begin{frame}[fragile]{轉存Excel檔案成CSV檔案}
\begin{enumerate}
\item 取得檔案：
\begin{itemize}
%\item [\texttt{2015RforBeginnerSlide.pdf}]本投影片
\item \texttt{exam.xlsx} 例範資料
\item \texttt{nation-data.xlsx} 練習資料
\end{itemize}
\item 在C disk 下創建一個\nbs\verb+LearnR2015+\nbs{}資料夾。\footnote{Unix-like電腦可放置於家目錄下的\texttt{LearnR2015}資料夾。}
\item 以Excel開啟\nbs\verb+exam.xlsx+，注意第一列必須是變數名稱。
\item 另存新檔 $\rightarrow$ 檔名為「exam」，類型為「CSV」，一樣儲存在\texttt{C:/LearnR2015}中。
\end{enumerate}
\end{frame}


\begin{frame}[fragile]{在R中讀取CSV資料檔案}
\begin{enumerate}
\item \verb+getwd()+\nbs{}顯示目前R所在的路徑。
...
\begin{frame}[fragile, allowframebreaks]{單樣本T檢驗}
...
\begin{frame}[fragile, allowframebreaks]{成對樣本T檢驗}
...
\begin{frame}[fragile, allowframebreaks]{獨立雙樣本T檢驗}
...
\end{itemize}
\begin{itemize}\item[Q] R畫的圖想做更多調整……
...
\begin{frame}{R適合你嗎？}
\begin{itemize}
\item R是很自由的語言，所以同一項任務給不同人寫可能會寫出非常不同的程式碼；其它統計軟體可能很制式，一步一步照著教材做。
\item R的學習梯度在初期較陡，必竟它也是一種程式語言。在學習後期梯度明顯較平緩。
\item 身為程式語言，R可以完成一般統計軟體不能辦到的「個人化」任務，但這在學習中後後期才出現。
\item R內建沒有圖形介面（但有第三方的支援軟體有），所以只能寫code。這對沒寫過程式的人可能很可怕。
\end{itemize}
\end{frame}
...
\begin{frame}{網路討論區}
\begin{itemize}
\item \href{https://www.ptt.cc/bbs/R_Language/index.html}{PTT的R\_Language板}\oldfootnote{\url{https://www.ptt.cc/bbs/R_Language/index.html}} \\ 路徑：戰略高手 $\rightarrow$ CompScience $\rightarrow$ R\_Language\\ 對初學者友善。
\item （中文的）\href{https://groups.google.com/forum/?hl=zh-TW\#!forum/taiwanruser}{R軟體使用者論壇}\oldfootnote{\url{https://groups.google.com/forum/?hl=zh-TW\#!forum/taiwanruser}}
\item \href{http://stackoverflow.com/questions/tagged/r}{Tag ``R''}  \makeatletter @ \makeatother stackoverflow.com\oldfootnote{\url{http://stackoverflow.com/questions/tagged/r}}
\end{itemize}
\end{frame}
...
\href{http://cran.r-project.org/web/packages/available_packages_by_name.html}{R官方套件庫}收錄有六千多個的套件，\footnote{\url{http://cran.r-project.org/web/packages/available_packages_by_name.html}}可直接以\nbs\verb+install.packages()+\nbs{}安裝。
\begin{block}{我常用的套件}
\begin{itemize}
...
\begin{frame}[fragile]{按今日課程試著完成以下練習}
\begin{enumerate}
...
% PDF to TIFFs using ghostscript
% gs -dNOPAUSE -sDEVICE=tiffgray -sCompression=lzw -r1800 -sOutputFile=pageTiff/p%03d.tiff 2015RforBeginnerSlide.pdf