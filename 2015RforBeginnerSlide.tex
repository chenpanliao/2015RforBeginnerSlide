...
\setsansfont[Mapping=tex-text, Scale=0.8, LetterSpace=-4, BoldFont={DejaVu Sans}]{DejaVu Sans}
\setmonofont[Color=codecolor, Scale=0.8, LetterSpace=-4, BoldFeatures={Color=white}, BoldFont={DejaVu Sans Mono}]{DejaVu Sans Mono}
...
\begin{block}{\texttt{t.test()}的基本語法}
\verb+t.test(資料1, 資料2, alternative = "t" 或 "l" 或 "g",+\\
\verb+       mu = 假說中平均數的差, var.equal = T 或 F, ...)+
...
{
\tiny 註：此例使用 \verb+t.test(..., var.equal = F)+ 可能較洽當，因為二組的變方差距不小。
}
\framebreak
